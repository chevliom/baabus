...
const ResultCount = () => {
	...
	return <div className="mt-4 font-[Poppins] text-lg font-medium text-gray-900 sm:ml-72 sm:text-xl">Showing {count} Results</div>;
};
		...
		<CategoryProvider>
			<div className="min-h-screen w-full bg-white">
				...
				<div className="relative mt-24 flex items-center justify-center h-[180px] w-full bg-[url(/image.png)] bg-cover bg-center sm:h-[219px]">
					<h1 className="absolute left-1/2 top-12 -translate-x-1/2 transform text-4xl font-normal text-[#ea518f] [font-family:'Baloo',Helvetica] sm:text-8xl">Product List</h1>
					<Image className="absolute left-4 top-0 h-auto w-28 object-cover sm:w-40" alt="Left" src="/image-71.png" width={160} height={211} priority />
					<Image className="absolute right-4 top-0 h-auto w-28 object-cover sm:w-40" alt="Right" src="/image-71.png" width={160} height={211} priority />
				</div>
										...
										<div key={option.value} className={`flex items-center gap-2 py-2.5 ${index === ratingOptions.length - 1 ? "pb-6" : ""}`}>
											...
											<label htmlFor={`rating-${option.value}`} className="flex items-center gap-2 cursor-pointer">
												{Array.from({ length: 5 }, (_, i) => (
													<Image key={i} src="/star-6.svg" alt="Star" width={15} height={14} className="h-[14px] w-[15px]" />
												))}
